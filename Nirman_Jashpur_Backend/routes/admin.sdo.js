...
router.get("/:id", auth, authorizeRole("Super Admin"), async (req, res) => {
  try {
    const doc = await SDO.findById(req.params.id);
...
router.put("/:id", auth, authorizeRole("Super Admin"), async (req, res) => {
  try {
    const doc = await SDO.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
    if (!doc) return res.status(404).json({ error: "Not found" });
...
router.delete("/:id", auth, authorizeRole("Super Admin"), async (req, res) => {
  try {
    const doc = await SDO.findByIdAndDelete(req.params.id);
    if (!doc) return res.status(404).json({ error: "Not found" });
